			    ...
			    unsigned int size, void *dest)
{
	...
	validcnt = (offs / conf->page_size) % conf->nseeds;
	...
	for (; size; size -= conf->page_size) {
		...
		if (ret < 0) {
			/*
			 * The ->nseeds value should be equal to the number
			 * of pages in an eraseblock. Since we don't know
			 * this information in advance we might have picked
			 * a wrong value.
			 * Fix the ->nseeds value and read the page again...
			 */
			if (validcnt)
				conf->nseeds = validcnt;

			if (nand_reset_column())
				return -1;

			/* ... if it still fails we have a real corruption. */
